...
| Runtime bundle diverges from reviewed source | Users execute unreviewed or stale code | Build guidance requires committing updated `dist/`; tests run against compiled source | Review `dist/index.js` diffs with source changes; consider CI that verifies `dist/` is current |
